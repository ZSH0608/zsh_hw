...
/**
 * Chart 2: Efficiency (Parameters vs Performance)
...
export function renderChart2(containerSelector, data) {
  ...
  // Highlight rings for specific efficient models
  //const efficientModels = ["Mistral 7B", "Llama 3 8B", "Llama 3 70B", "Claude 3.5 Sonnet", "Mixtral 8x7B", "Claude 3 Sonnet", "o4-mini", "Llama 4 Scout"];
  ...
  console.log(`高效模型数量 (E > ${EFFICIENCY_THRESHOLD}):`, efficientModels.length, efficientModels.map(d => d.name));
  ...
  // Labels for large models or efficient ones
  /*
  const labelData = chartData.filter(d => d.parameters > 1e12 || efficientModels.includes(d.name));
  
  g.selectAll(".label")
    .data(labelData)
    .enter().append("text")
    .attr("x", d => xScale(d.parameters) + 8)
    .attr("y", d => yScale(d.mmluScore) + 4)
    .text(d => d.name)
    .attr("font-size", "10px")
    .attr("fill", "#475569")
    .style("pointer-events", "none")
    .attr("opacity", d => (!useLogScale && d.parameters < 200e9) ? 0 : 1);*/
}